...
router.post("/add", async (req, res) => {
  ...
  if (error)
    return res
      .status(400)
      .send(functions.getResponse(0, error.details[0].message, []));

  let product = new Product(
    _.pick(req.body, ["name", "description", "image_url", "cost", "available"])
  );
...
router.put("/:id", async (req, res) => {
  ...
  if (!product)
    return res.status(404).send(functions.getResponse(0, "recordNotFound", []));
...
router.delete("/:id", async (req, res) => {
  ...
  if (!product)
    return res
      .status(404)
      .send(functions.getResponse(0, "ProductNotFound", []));
